...
/*pcl::PointCloud<pcl::PointXYZ>::Ptr BiggestCluster(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, double tolerance, bool& flag) {
	...
	if (npoints.size() == 0) {
		...
		maxindex = 0;
		for (std::vector<unsigned int>::iterator it = npoints.begin(); it != npoints.end(); ++it) {
			if (*it > maxpoints) {
				maxpoints = *it;
				maxindex = i;
			}
...
/*void PasstroughFilterXYZ(const pcl::PointCloud<pcl::PointXYZ>::Ptr& in, pcl::PointCloud<pcl::PointXYZ>::Ptr& out, double lowboxlimits[3], double upperboxlimits[3]) {
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_pointsX(new pcl::PointCloud<pcl::PointXYZ>);
...
/*std::vector<std::vector<float>> VectorPlanesFitted(const pcl::PointCloud<pcl::PointXYZ>::Ptr RawCloud, int& maxindexp) {
	...
	if (npointsplane.size() == 0) {
		...
		maxindexp = 0;
		for (std::vector<unsigned int>::iterator it = npointsplane.begin(); it != npointsplane.end(); ++it) {
			if (*it > maxpointsp) {
				maxpointsp = *it;
				maxindexp = i;
			}
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr PlanarRANSAC(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, double &zPLANE)
{
	...
	pass.setFilterLimits((cloud->points[inliers->indices[0]].z -5), (cloud->points[inliers->indices[0]].z + 0.002));
	//pass.setFilterLimitsNegative (true);
