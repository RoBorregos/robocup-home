...
class Detect3DFloor
{
  ...
  const std::string name = "Detect3DFloor";
  ros::NodeHandle nh_;
...
public:
    ...
    as_(nh_, name, boost::bind(&Detect3DFloor::handleActionServer, this, _1), false)
  {
    ...
    ROS_INFO_STREAM("Action Server Detect3DFloor - Initialized");
    // Load Params.
    nh_.param("/Detection3DBag/BASE_FRAME", BASE_FRAME, BASE_FRAME);
    nh_.param("/Detection3DBag/CAMERA_FRAME", CAMERA_FRAME, CAMERA_FRAME);
    nh_.param("/Detection3DBag/POINT_CLOUD_TOPIC", POINT_CLOUD_TOPIC, POINT_CLOUD_TOPIC);
  ...
  {
    ROS_INFO_STREAM("Action Server Detect3DFloor - Goal Received");
    ...
    {
      ROS_INFO_STREAM("Action Server Detect3DFloor - Preempted");
  ...
  void cloudCB(const sensor_msgs::PointCloud2& input)
  {
    ...
    // Detect and Remove the floor on which the object is resting.
...
{
  // Initialize ROS
  ros::init(argc, argv, "detect_objects_in_floor");

  // Start the segmentor
  Detect3DFloor segmentor;
