...
from arm_server.msg import img, img_list, MoveHRI
import uuid
...
class FaceDetection():
    ...
    def __init__(self):
        ...
        self.move_pub = rospy.Publisher("move", MoveHRI, queue_size=1)
        self.image = None
    ...
    def run(self):
        ...
        while rospy.is_shutdown() == False :
            ...
            self.move_pub.publish(move)
            imglist = img_list()
            imglist.images = img_arr
            imglist.len = len(img_arr)
            self.detection_pub.publish(imglist)
