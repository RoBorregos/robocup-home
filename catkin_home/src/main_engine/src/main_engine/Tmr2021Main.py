...
class Tmr2021Main(object):
    ...
    def listen_parser(self, bring_something_cmd):
        if self.currentState != SPEECH_STATE:
            return
        self.targetPlace = bring_something_cmd.place.lower()
        self.targetObject = bring_something_cmd.object.lower()
        self.talkerDashgo.publish(self.targetPlace)
        self.currentState = NAVIGATION1_STATE

    def run(self):
        while self.currentState == START_STATE or self.currentState == SPEECH_STATE:
