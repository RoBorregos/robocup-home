...
obj_det_home_path = path.join(rp.get_path(
    "action_selectors"), "..", "..", "..", "object_detection")

COMMAND_OBJ_DETECTION = path.join(
    obj_det_home_path, "venv", "env", "bin", "activate")
COMMAND_OBJ_DETECTION += "; "
COMMAND_OBJ_DETECTION += path.join(obj_det_home_path,
                                   "venv", "env", "bin", "python3")
COMMAND_OBJ_DETECTION += " "
COMMAND_OBJ_DETECTION += path.join(obj_det_home_path,
                                   "scripts", "get_objects_and_coordinates_in_terminal.py")
...
def callback_timer_analyze_msg_image(_):
    ...
    if USE_RS_CAMERA:
        ...
        cv_image = cv_bridge.imgmsg_to_cv2(
            msg_img, desired_encoding="passthrough")
    ...
    with tempfile.NamedTemporaryFile(suffix=".jpg") as temp_img_file:
        ...
        temp_img_file.flush()
        rospy.loginfo(
            "Tmp image saved (" + temp_img_file.name + "), calling script")

        while True:
            if obj_det_process.stdout.readline().decode('utf-8') == '~#waiting input#~\n':
                break
        obj_det_process.stdin.write(
            (temp_img_file.name + '\n').encode('utf-8'))
    ...
    if USE_RS_CAMERA:
        ...
        depth_image = cv2.resize(depth_image, RE_SIZE,
        ...
        for obj in objects[0:2]:
            ...
            center_y = center_y * (1 + 1.0 * (90 - angle_z_y) /
                                   90.0) if angle_z_y > 0 else center_y * (1 + 1.0 * (90 + angle_z_y) / 90.0)
            center_y = int(center_y)
            four_limits_obj = Segmentation.depth_trace_limits_of_four_sides_of_object(
                cv_image, depth_image, obj, rs_intrinsics, center_x_y=(
                    center_x, center_y),
...
def callback_shutting_down():
    ...
    rospy.loginfo(
        "*Received signal, node will finish (only if it finished setting up)*")
...
def main():
    ...
    publisher = rospy.Publisher(
        "objects_detected", ObjectsDetected, queue_size=RATE)
    ...
    seconds = int(1/RATE)
    timer_analyze = rospy.Timer(rospy.Duration(seconds, int(
        1000000000*(1.0/RATE - seconds))), callback_timer_analyze_msg_image)
    ...
    if USE_RS_CAMERA:
        ...
        depth_info = rospy.wait_for_message(
